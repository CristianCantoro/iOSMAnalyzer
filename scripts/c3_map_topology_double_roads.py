...
#description     :This file creates a map: Calculates all currently "valid" double road geometries
...
pic_output_name = 'pics/c3_map_topology_double_roads'
...
SELECT geom FROM
	...
	WHERE
		...
		AND minor = (SELECT max(minor) FROM hist_line AS h WHERE h.id = hist_line.id AND h.version = hist_line.version AND
			(valid_from <= CURRENT_TIMESTAMP AND (valid_to >= CURRENT_TIMESTAMP OR valid_to is null)))) AND tags ? 'highway' AND visible = 'true'
	GROUP BY geom 
...
line_symbolizer = mapnik.LineSymbolizer(mapnik.Color('#f5001d'),4)
