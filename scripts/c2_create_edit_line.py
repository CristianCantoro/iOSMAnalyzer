...
LEFT   JOIN hist_line t ON t.valid_from::date = s.day AND t.version = 1 AND t.minor = 0
GROUP  BY month
ORDER  BY month;
...
LEFT   JOIN hist_line t ON t.valid_from::date = s.day AND NOT (t.version = 1 AND t.minor = 0) AND visible = 'true'
GROUP  BY month
ORDER  BY month;
...
plt.plot(dates, col1_1, color = '#2dd700', linewidth=2, label='Created Lines')
plt.plot(dates, col1_2, color = '#ff6700', linewidth=2, label='Line-Edits')
...
# Label x and y axis
plt.xlabel('Date')
plt.ylabel('Number of created Lines and Line-Edits')
...
# Plot-title
plt.title("Development of created Lines and Line-Edits")

# Save plot to *.png-file
plt.savefig('pics/c2_create_edit_line.jpeg')
